...
/**
 * @brief 
 * 本来管理的Heap的大小是固定的，无法动态增长。
 * Heap是一个数组，因此需要指定元素类型(描述符堆类型)、数组长度。HANDLE就是一个指针。
 * 有了堆的起始内存地址，根据元素大小，可以求出任一元素的地址。描述符堆类型一共有4种(各种View以及Sampler)。
 * 
