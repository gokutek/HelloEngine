...
color_buffer::color_buffer(color clear_color) :
...
void color_buffer::create(wchar_t const* name, uint32_t width, uint32_t height, uint32_t mip_level, DXGI_FORMAT format, D3D12_GPU_VIRTUAL_ADDRESS gpu_va)
{
	...
	create_texture_resource(graphics::device, name, desc, clear_value, gpu_va);
	create_derived_views(graphics::device, format, 1, mip_level);
}
